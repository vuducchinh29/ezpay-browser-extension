        ...
        return (
            <div className={`container ${modeCssName}`}>
                ...
                <div className="history scroll">
                    {this.renderLoading()}
                    {(histories && histories !== 'nodata' && histories.length > 0) && <Accordion className="my-accordion" onChange={this.onChange}>
                        {histories.map((tx) => (
                            <Accordion.Panel accordion={true} header={
                                <div className={`header-item ${modeCssName}-history-title`}>
                                    <div>{moment.unix(tx.timeStamp).format('MMMM Do YYYY, hh:mm:ss')}</div>
                                    <div className="value">{tx.value} {account.symbol}</div>
                                </div>}>
